...
def select_municipio(df,municipio):
    ...
    groupby(["date"])[["temp_min","temp_max"]].mean().reset_index().sort_values(by="date",ascending=False).reset_index(drop=True)
    return df
...
# Load your data
df = pd.read_csv("data/03_primary/Temperature_data.csv")
df['date'] = pd.to_datetime(df['date'])  # Ensure 'date' column is datetime type
df["departamento"]=df["departamento"].str.strip()
df["departamento"]=df["departamento"].str.replace("SAN ANDRES Y  PROVIDENDIA","SAN ANDRES Y PROVIDENDIA")
#df["departamento"].loc["departamento"= "SAN ANDRES Y  PROVIDENDIA"]
...
app.layout = html.Div([
    ...
    html.Div([
        html.P(["Explore la evolución de las temperaturas máximas y mínimas de los municipios de Colombia. Configure los filtros y dele click en 'Play' para visualizar los datos. Desarrollado con datos del IDEAM por ",
        html.A("David Alejandro López Atehortúa." , href="www.linkedin.com/in/davidalopeza", target="_blank"),
    ...
    html.Div([
        ...
        ], style={'marginLeft': '5', 'width': '290px', 'height': '10px','display': 'inline-block'}),
        ...
        ],style={'fontSize': '19px', 'height': '50px','width': '190px','marginDown':'5px','marginLeft':'15px'}),
        ...
        ], style={'marginLeft': '10px', 'width': '210px', 'display': 'inline-block'}),
        ...
        ], style={'marginDown': '10px','width': '210', 'display': 'inline-block','height': '65px'}),
